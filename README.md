...
This package is ready for a Render **Web Service**. Push it to a Git repository, create a new Blueprint in Render, and select the repository. Render reads `render.yaml`, installs dependencies, builds the Node/Nitro server, and starts it on the platform-provided `PORT`.

Before the first deploy, set the values marked `sync: false` in Render (copy the names and guidance from `.env.example`). At minimum, configure `GEMINI_API_KEY`, the five Supabase values, and `APP_URL` with the final Render URL. `GEMINI_API_KEY` is server-only; never prefix it with `VITE_`.

For local development, copy `.env.example` to `.env`, fill in the values, then run:
...
The application calls the Google Gemini API directly for ticket classification, generated ticket replies, chatbot replies, and admin analytics. It falls back gracefully when a Gemini key is not configured or the API is unavailable.
...
- 
- **Database**: Supabase PostgreSQL with a `tickets` table — `user_name`, `title`, `details`, `category`, `priority`, `created_at`.
- 
- **AI Classification Flow**: On submit, the server function sends the ticket title + details directly to Google Gemini (`gemini-2.5-flash`) with a strict JSON prompt that returns `{ categories, priority }`. If Gemini is unavailable, a keyword-based heuristic (e.g. "urgent" → High, "vpn/laptop" → IT) provides a graceful fallback.
...
React • Tailwind CSS • TanStack Start • Supabase • Google Gemini
---
