...
import { sendTicketEmailSafe, type TicketEmailInput } from "./email";
...
/** Calls Google Gemini directly. GEMINI_API_KEY never reaches the browser. */
...
async function classifyWithAI(title: string, details: string) {
  try {
    ...
    if (!content) return heuristicClassify(title, details);
...
export const generateTicketResponse = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      const content = await generateGeminiText(system, [
        {
          role: "user",
...
export const askTicketBot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ reply: string }> => {
    ...
    if (!process.env.GEMINI_API_KEY) {
      ...
      const history: GeminiMessage[] = (notes ?? []).slice(-20).map((n) => ({
        role: n.author_role === "ai" ? "model" : "user",
        text:
          n.author_role === "admin"
      ...
      try {
        aiResponse =
          (await generateGeminiText(system, history)) ??
