...
async function generateGeminiText(system: string, prompt: string, jsonMode = false): Promise<string | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  // Kept in sync with tickets.server.ts — Google has been retiring Gemini
  // models faster than their published shutdown dates in 2026 (2.0 Flash,
  // 1.5, 1.0 already gone; 2.5 Flash has been reported 404ing ahead of its
  // official Oct 16 2026 date too). GEMINI_MODEL env var overrides this
  // without a redeploy if Google moves the goalposts again.
  ...
  if (!response.ok) {
    // Log instead of silently falling back — otherwise a bad model name,
    // expired key, or quota issue looks identical to "AI is just off".
    const errText = await response.text().catch(() => "");
    console.error(`Gemini API error ${response.status} for model "${model}" (analytics): ${errText}`);
    return null;
  }
...
export const generateInsightsReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data: range }) => {
    ...
    if (!process.env.GEMINI_API_KEY) return { summary, narrative: fallbackNarrative, source: "fallback" as const };
...
export const generateDeepInsights = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data: range }): Promise<DeepInsights> => {
    ...
    if (!process.env.GEMINI_API_KEY || rows.length === 0) return fallback;

    try {
      const text = await generateGeminiText(
        "You analyse support-ticket data and return STRICT JSON only — no prose or markdown fences. Return an object with common_issues (array of {theme, count, example}), common_fixes (array of {fix, applies_to}), time_analysis (string), business_insights (string array), and recommendations (string array). Identify recurring issue themes, common fixes used by admins, time/resolution patterns, and business insights a department head would care about. Be specific and reference actual ticket patterns from the data.",
        `Scope: ${scope}. Stats: ${JSON.stringify(stats)}. Tickets sample: ${JSON.stringify(sample)}`,
        true,
      );
      if (!text) return fallback;
      const parsed = JSON.parse(text) as Omit<DeepInsights, "scope" | "generated_at" | "source" | "stats">;
