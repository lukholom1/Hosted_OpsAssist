...
export const Route = createFileRoute("/_authenticated/admin/tickets")({
  ...
  validateSearch: (s: Record<string, unknown>): { ticket?: string; focus?: string } => ({
...
function AdminTicketsPage() {
  ...
  const { department, role, ready, isSuperAdmin } = useAuth();
  useEffect(() => {
    if (ready && role && role !== "admin") navigate({ to: "/dashboard", replace: true });
  }, [ready, role, navigate]);
