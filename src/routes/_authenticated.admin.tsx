...
function AdminPage() {
  ...
  const { signOut, fullName, department, role, ready, isSuperAdmin, can } = useAuth();
  useEffect(() => {
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-10">
        ...
        <div className="flex flex-col gap-3">
          <div className="inline-flex w-fit items-center gap-1.5 rounded-full border border-border bg-card/50 px-2.5 py-1 text-[11px] font-medium text-muted-foreground">
            ...
            {!ready ? "Workspace" : isSuperAdmin ? "SuperAdmin console" : `${department} workspace`}
          </div>
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">
            {!ready || isSuperAdmin ? "Admin dashboard" : `${department} dashboard`}
          </h1>
